...
# start the measure
class EnableDetailedOutputForEachNodeInALoop < OpenStudio::Measure::ModelMeasure
  # human readable name
  def name
...
- Which variable they want to output for each node:
  ...
  def arguments(model)
    ...
    loop = OpenStudio::Measure::OSArgument.makeChoiceArgument('loop', loop_handles, loop_display_names, true)
    ...
    varhash.each do |k, v|
      newarg = OpenStudio::Measure::OSArgument.makeBoolArgument(k, true)
  ...
  def run(model, runner, user_arguments)
    ...
    if loop.empty?
      ...
      if !loop.get.to_AirLoopHVAC.empty?
      ...
      elsif !loop.get.to_PlantLoop.empty?
    ...
    if include_demand_nodes
      ...
      node_names = add_node_names_to_array(dm_objects, node_names)
    end
    ...
    node_names.each do |node_name|
      ...
      variable_names.each do |variable_name|
        out_var = OpenStudio::Model::OutputVariable.new(variable_name, model)
        out_var.setReportingFrequency(reporting_frequency)
        out_var.setKeyValue(node_name)
        puts out_var
        puts "\n"
      end
    end
